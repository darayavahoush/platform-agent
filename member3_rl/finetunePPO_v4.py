...
FINETUNE_EPISODES = 1000    # round 4: continuing the same locked-difficulty regime
MAX_STEPS = 300
ROLLOUT_EPISODES = 10
EVAL_EVERY = 50
EVAL_EPISODES = 20
...
LOAD_PATH = os.path.join(MODEL_DIR, "ppo_platform_agent_finetuned_v3_best.pth")  # round 3 result
FT_BEST_PATH = os.path.join(MODEL_DIR, "ppo_platform_agent_finetuned_v4_best.pth")
FT_FINAL_PATH = os.path.join(MODEL_DIR, "ppo_platform_agent_finetuned_v4_final.pth")

# Continue from a seed range that does not overlap prior training (42..2542)
# or the rigorous eval (999000..999100).
BASE_SEED = 800000  # non-overlapping with rounds 1-3 (500000+, 600000+, 700000+) and rigorous eval (999000+)
...
def finetune():
    ...
    print("=" * 65)
    print("     PPO FINE-TUNE ROUND 4: LOCKED FULL DIFFICULTY (continued)")
    print("=" * 65)
    ...
    while episode < FINETUNE_EPISODES:
        ...
        if episode % EVAL_EVERY == 0:
            eval_reward, eval_success = evaluate(
                policy, start_seed=1000000 + episode, episodes=EVAL_EPISODES
            )
