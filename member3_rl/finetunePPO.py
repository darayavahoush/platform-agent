...
FINETUNE_EPISODES = 800     # additional episodes, all at full difficulty
MAX_STEPS = 300
ROLLOUT_EPISODES = 10
EVAL_EVERY = 50
EVAL_EPISODES = 20
...
LOAD_PATH = os.path.join(MODEL_DIR, "ppo_platform_agent_best.pth")
FT_BEST_PATH = os.path.join(MODEL_DIR, "ppo_platform_agent_finetuned_best.pth")
FT_FINAL_PATH = os.path.join(MODEL_DIR, "ppo_platform_agent_finetuned_final.pth")
...
def finetune():
    ...
    print("=" * 65)
    print("     PPO FINE-TUNE: LOCKED FULL DIFFICULTY (no curriculum)")
    print("=" * 65)
    ...
    while episode < FINETUNE_EPISODES:
        ...
        if episode % EVAL_EVERY == 0:
            eval_reward, eval_success = evaluate(
                policy, start_seed=700000 + episode, episodes=EVAL_EPISODES
            )
